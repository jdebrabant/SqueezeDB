...
def buildQueriesHist(hfile):
    ...
    with open(hfile, 'rt') as hFILE:
        ...
        while True:
            ...
            if queries[-1].ops[-1].table == " a_k5_u200k_unif a  ":
                aSelect = queries[-1].ops[-1]
                queries[-1].ops[-1] = queries[-1].ops[-2]
                queries[-1].ops[-2] = aSelect
            ...
            lines[1] = hFILE.readline()
            lines[2] = hFILE.readline()
    return queries
...
def compareQueries(lqueries, hqueries, lsize, e):
    ...
    for i in range(len(lqueries)):
        ...
        s.ops[0].err = math.fabs(s.ops[0].estRows - l.ops[0].actRows) / math.pow(lsize, 2)
        ...
        if l.ops[0].actRows > 0:
            ...
            s.ops[0].errPerc = (s.ops[0].err * 100) /  (l.ops[0].actRows / math.pow(lsize, 2))
        else:
        ...
        s.ops[1].err = math.fabs(s.ops[1].estRows - l.ops[1].actRows) / lsize

        if l.ops[1].actRows > 0:
            l.ops[1].errPerc = (l.ops[1].err * 100) /  (l.ops[1].actRows / lsize)
            s.ops[1].errPerc = (s.ops[1].err * 100) /  (l.ops[1].actRows / lsize)
        else:
            if l.ops[1].estRows > 0:
                l.ops[1].errPerc = (l.ops[0].err * 100) /  (1 / math.pow(lsize, 2))
            else:
                l.ops[1].errPerc = 0
            if s.ops[1].estRows > 0:
                s.ops[1].errPerc = (s.ops[0].err * 100) /  (1 / math.pow(lsize, 2))
            else:
                s.ops[1].errPerc = 0
    ...
    for i in range(len(lqueries)):
        ...
        errpercpredsum += math.pow(s.ops[1].errPerc - results['avgerrpercpred'], 2)
