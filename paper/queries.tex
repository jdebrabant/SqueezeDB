...
$T=Q/A$, where $Q$ is a \emph{quadratic} function of the variables, and $A$ is
a \emph{positive affine} function of the variables. Then their ratio is
convex~\citep{BoydV04}. Consider the minimization problem using $T$ as an
objective function, and let $T'^-$ be the optimal solution to this problem. This
optimization problem can be solved in polynomial time.
\question{One can actually replace $A$ with an upper bound and have a quadratic
optimization problem. Shall we mention that?}
We have
\[
...
known to be a NP-hard problem even under the mildest conditions, but it is known
that the optimal solution can be found at one of the extremal points of the
polytope. Thanks to this and other interesting mathematical properties, there
are a number of algorithms try to solve this problem as efficiently as
possible~\citep{Benson95},\citemissing.
\todo{complete.}

\todo{I want to check what~\citet{Haas96} does for maximization of the variance.}
