...
(think SUM) and and that we may use different estimators.}
...
option: \todo{Here we should explain that we cannot use these because we would
have to apply a union bound over a very high number of queries, defeating the
purpose.}. Our approach instead guarantees that, if the sample $\Sam$ is an
...
\end{equation}
\question{Is that $\le$ and $\ge$ ?}
Clearly $q^*_{v_i}=q^*_{i,i}$ for all $v_i\in V_{q^*}(C_f)$ \itodo{$\Leftarrow$ Adjust the above definition so that this is true.}. Let now
...
efficiently. 
\todo{Say how many there are and perhaps explain the presence of $\min$ and
$\max$ .}
...
of statistics stored by the DMBS for query optimization\iquestion{Haas calls
this the ``system catalog'', is that correct?} Consider the optimization
...
polynomial time.
\todo{What I'd like to argue is that (in many cases) the optimal solution to the
...
\todo{I'm trying to understand what we can do here and whether this is a
(quasi-)convex function or what or whether we can at least lower/upper bound it
by a (quasi)-convex function.}
...
\todo{DBMS's actually have two functions for variance and two function for
...
For median \todo{Take care of median. I don't really know much about it, and
